    ...
    public static void processConversationsForUI(List<Conversation> conversations, String currentUserId) {
        for (Conversation conversation : conversations) {
            if (conversation.isOneOnOneChat()) {
                ...
                Log.d("ChatHelper", "Other Participant Name after compute: " + conversation.getOtherParticipantName());
                // Compute other participant ID
                conversation.computeOtherParticipant(currentUserId);
                Log.d("ChatHelper", "Other Participant Name: " + conversation.getOtherParticipantName());

                // TODO: In a real app, you would make API calls here to fetch user details
                // For now, we'll use placeholder logic
                if (conversation.getOtherParticipantId() != null) {
                    // Placeholder: In real app, call API to get user info
                    String otherParticipantName = generatePlaceholderName(conversation.getOtherParticipantId());
                    String avatarInitials = AvatarUtils.generateAvatarInitials(otherParticipantName);

                    conversation.setOtherParticipantName(otherParticipantName);
                    conversation.setOtherParticipantAvatar(avatarInitials);
                    conversation.setDisplayName(otherParticipantName);
                    conversation.setAvatar(avatarInitials);
                }
            }
        }
    }
